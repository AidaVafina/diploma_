...
Теперь в интерфейсе и API доступны три режима обработки:

- `full` — текущая полная обработка: рендер, OCR-предобработка, layout, OCR текстовых блоков и формулы;
- `no_preprocessing` — тот же конвейер, но без OCR-предобработки, для документов хорошего качества;
- `text_only` — ускоренный режим только для извлечения текста: без layout, без формул и без OCR-предобработки.
...
- `file` — PDF-файл;
- `processing_mode` — необязательный режим обработки (`full`, `no_preprocessing`, `text_only`), по умолчанию `full`.

Дальше автоматически запускает:
...
В режиме `text_only` сервис пропускает layout-анализ, OCR формул и разбиение на статьи, а для страниц со встроенным текстом вообще не рендерит изображение.
