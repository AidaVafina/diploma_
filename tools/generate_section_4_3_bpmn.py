...
OUT_DIR = ROOT / "artifacts" / "section_4_3"

PNG_PATH = OUT_DIR / "text_block_ocr_module_bpmn.png"
SVG_PATH = OUT_DIR / "text_block_ocr_module_bpmn.svg"
PNG_SLIDE_PATH = OUT_DIR / "text_block_ocr_module_bpmn_slide.png"
SVG_SLIDE_PATH = OUT_DIR / "text_block_ocr_module_bpmn_slide.svg"
...
def draw_title(draw: ImageDraw.ImageDraw) -> None:
    draw.text((140, 60), "Модуль OCR текстовых блоков в формате BPMN", font=FONT_TITLE, fill=TEXT)
    draw.text(
        (140, 136),
        "PaddleOCR для routed text-блоков, fallback на seed_text и правила needs_review",
        font=FONT_SUBTITLE,
        fill=MUTED,
    )
...
def draw_pool(draw: ImageDraw.ImageDraw, *, offset_y: int) -> tuple[int, int, int, int]:
    pool = (140, 280 + offset_y, W - 140, 1500 + offset_y)
    header_w = 190
...
def draw_gateway(draw: ImageDraw.ImageDraw, gateway: Gateway) -> None:
    ...
    draw.text((cx - 14, cy - 21), symbol, font=FONT_GATEWAY, fill=TEXT)
...
def draw_flow(draw: ImageDraw.ImageDraw, flow: Flow) -> None:
    ...
    draw.polygon(
        arrow_head(last_end, (last_end[0] - last_start[0], last_end[1] - last_start[1])),
        fill=LINE,
    )
...
def base_events() -> list[Event]:
    return [
        Event("start", 320, 900, 42, "start", "Старт"),
        Event("end", 5230, 580, 46, "end", "Конец"),
    ]
...
def base_gateways() -> list[Gateway]:
    return [
        Gateway("route", 1420, 900, 46, "exclusive"),
        Gateway("ocr_split", 2460, 1220, 46, "exclusive"),
        Gateway("ocr_join", 3000, 1220, 46, "exclusive"),
    ]


def base_flows() -> list[Flow]:
    return [
        Flow(points=((362, 900), (460, 900))),
        Flow(points=((820, 900), (960, 900))),
        Flow(points=((1320, 900), (1420, 900)), label="цикл по блокам", label_xy=(1280, 852)),
        Flow(points=((1466, 900), (1466, 600), (1510, 600)), label="нет", label_xy=(1450, 690)),
        Flow(points=((1420, 946), (1420, 1220), (1510, 1220)), label="да", label_xy=(1350, 1080)),
        Flow(points=((1880, 1220), (2020, 1220))),
        Flow(points=((2390, 1220), (2460, 1220))),
        Flow(points=((2506, 1220), (3000, 1220)), label="да", label_xy=(2665, 1178)),
        Flow(
            points=((2460, 1266), (2460, 1390), (2520, 1390)),
            label="ошибка / unavailable",
            label_xy=(2210, 1355),
        ),
        Flow(points=((2890, 1390), (3000, 1390), (3000, 1220))),
        Flow(points=((3046, 1220), (3130, 1220))),
        Flow(points=((3500, 1220), (3640, 1220))),
        Flow(points=((3825, 1090), (3825, 730))),
        Flow(points=((1880, 600), (3640, 600))),
        Flow(points=((4010, 580), (4180, 580)), label="после всех блоков", label_xy=(4040, 532)),
        Flow(points=((4540, 580), (4680, 580))),
        Flow(points=((5060, 580), (5184, 580))),
    ]
...
def svg_gateway(gateway: Gateway) -> str:
    ...
    return (
        ...
        f'fill="{TEXT}" font-family="Arial">{escape("X")}</text>'
    )
...
def render_variant(*, with_title: bool) -> tuple[Image.Image, str]:
    ...
    if with_title:
        svg_parts.append(
            ...
            f'{escape("Модуль OCR текстовых блоков в формате BPMN")}</text>'
        )
        svg_parts.append(
            f'<text x="140" y="168" font-size="{SUBTITLE_FONT_SIZE}" fill="{MUTED}" font-family="Arial">'
            f'{escape("PaddleOCR для routed text-блоков, fallback на seed_text и правила needs_review")}</text>'
        )
    ...
    svg_parts.append(
        f'<line x1="{pool[0] + 190}" y1="{pool[1]}" x2="{pool[0] + 190}" y2="{pool[3]}" stroke="{POOL_BORDER}" stroke-width="3" />'
    )
    pool_lines = ["Модуль", "OCR", "текстовых", "блоков"]
    lh = line_height(draw, FONT_POOL)
